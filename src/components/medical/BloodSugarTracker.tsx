...
export function BloodSugarTracker() {
  ...
  async function fetchRecords() {
    ...
    try {
      setIsLoading(true)
      const response = await fetch(`https://${projectId}.supabase.co/functions/v1/make-server-6dba5bdd/blood-sugar`, {
        headers: {
      ...
      if (response.ok) {
        ...
        setRecords((records || []).sort((a: BloodSugarRecord, b: BloodSugarRecord) => 
          new Date(b.measuredAt).getTime() - new Date(a.measuredAt).getTime()
        ))
      } else {
        throw new Error('혈당 기록을 불러올 수 없습니다')
      }
    } catch (error) {
      console.error('혈당 기록 조회 중 오류:', error)
      setError(error instanceof Error ? error.message : '혈당 기록 조회 실패')
    } finally {
      setIsLoading(false)
    }
  }
  ...
  async function createRecord() {
    ...
    try {
      ...
      if (response.ok) {
        ...
        throw new Error(errorData.error || '혈당 기록 실패')
      }
    } catch (error) {
