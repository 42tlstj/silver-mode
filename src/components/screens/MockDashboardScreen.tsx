...
import { MockAppointmentManager } from '../medical/MockAppointmentManager';
import { MockMedicationTracker } from '../medical/MockMedicationTracker';
import { MockBloodSugarTracker } from '../medical/MockBloodSugarTracker';
import { 
  Calendar, 
  Pill, 
  Activity, 
...
interface MockDashboardScreenProps {
  onNavigate: (screen: string) => void;
}

export function MockDashboardScreen({ onNavigate }: MockDashboardScreenProps) {
  ...
  const tabs = [
    ...
    ...(user?.diseases?.includes('diabetes') ? [{ id: 'bloodsugar', label: '혈당 관리', icon: Activity }] : [])
  ];
  ...
  const renderTabContent = () => {
    switch (activeTab) {
      ...
      case 'medications':
        return <MockMedicationTracker />;
      case 'bloodsugar':
        return <MockBloodSugarTracker />;
      default:
            ...
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600">
                  {user?.diseases?.map(d => getDiseaseLabel(d)).join(', ')} 관리 모드
                </p>
              </div>
            </div>
          </div>
